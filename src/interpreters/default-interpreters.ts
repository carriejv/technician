...
export class DefaultInterpreters {
     ...
     * @param encoding The text encoding to use. Default `utf8`.
     */
    public static asJSON(encoding: SupportedEncoding = 'utf8'): Interpreter<JSONData | undefined> {
        return async (rawEntity: RawConfigEntity) => {
     ...
     * @param encoding The text encoding to use. Default `utf8`.
     */
    public static asTextOrJSON(encoding: SupportedEncoding = 'utf8'): Interpreter<JSONData | string> {
        return async (rawEntity: RawConfigEntity) => {
            const text = rawEntity.data.toString(encoding);
            try {
                return JSON.parse(text);
